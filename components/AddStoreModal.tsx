...
import { XIcon, availableColors, colorClassMap } from '../constants';
...
const ColorSwatch: React.FC<{ color: string; isSelected: boolean; onClick: () => void }> = ({ color, isSelected, onClick }) => {
    ...
    return (
        <button
            type="button"
            onClick={onClick}
            className={`w-10 h-10 rounded-full border-4 ${colorClasses.bg500} ${isSelected ? `${colorClasses.border700} ring-2 ring-offset-2 ring-offset-surface ${colorClasses.ring700}` : 'border-surface'} transition-all`}
...
const StoreModal: React.FC<StoreModalProps> = ({ storeToEdit, onClose, onSave }) => {
  ...
  useEffect(() => {
    if (storeToEdit) {
      ...
      setSelectedColor(storeToEdit.color);
    } else {
      // Reset for new store
      setName('');
      setSelectedColor(availableColors[Math.floor(Math.random() * availableColors.length)]);
    }
  }, [storeToEdit]);
      ...
      <div className="bg-surface rounded-2xl border border-border w-full max-w-sm m-4 animate-fade-in-up shadow-card" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit}>
          <div className="p-6 space-y-6">
            ...
            <div>
                <label className="block text-sm font-medium text-ink-muted mb-2">Color de la Tienda</label>
